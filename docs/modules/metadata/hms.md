...
Hive Metastore (HMS) provides a central repository of metadata for tables stored in a data lake.
...
|------|-------------|------|---------|:--------:|
| <a name="input_additional_jars"></a> [additional\_jars](#input\_additional\_jars) | Map of filename to URL for additional JARs | `map(string)` | `default` | no |
...
| <a name="input_hive_metastore_warehouse_dir"></a> [hive\_metastore\_warehouse\_dir](#input\_hive\_metastore\_warehouse\_dir) | Hive Metastore warehouse directory | `string` | `"s3a://datalake/warehouse"` | no |
| <a name="input_hive_site_config"></a> [hive\_site\_config](#input\_hive\_site\_config) | Additional configuration properties for hive-site.xml | `map(string)` | `default` | no |
...
| <a name="input_resources_config"></a> [resources\_config](#input\_resources\_config) | Resource requests/limits | `object` | `{}` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_config"></a> [config](#output\_config) | Complementary configuration object containing the internal URL. |
| <a name="output_ingress_host"></a> [ingress\_host](#output\_ingress\_host) | The external hostname of HMS |
