...
### Changed
- **ClickHouse UI**: Renamed container to `chouse-ui` and updated image reference to `ghcr.io/daun-gatal/chouse-ui`.
- **Documentation**: Updated Lab Environment link for ClickHouse UI.

## [v0.8.0] - 2026-01-10

### Added
- **ClickHouse UI**: Added `env_from_secrets` variable (list of strings) to auto-inject all environment variables from specified Kubernetes secrets.

## [v0.7.0] - 2026-01-08

### Changed
- **BREAKING CHANGE**: `clickhouse/ui` - Replaced `clickhouse_urls` variable with flexible `env_vars` map for container environment variables.
- **BREAKING CHANGE**: `clickhouse/ui` - Removed `image_repository` variable.

### Migration
- Update `clickhouse_urls = "http://..."` to `env_vars = { VITE_CLICKHOUSE_URLS = "http://..." }`
